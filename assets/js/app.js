(() => {
  ...
  const validRoutes = new Set(["home", "research", "publications", "awards", "media", "contact"]);
  ...
  function mediaPostEmbed(post) {
    const activityId = linkedinActivityId(post.url);

    if (!activityId) {
      return `<article class="linkedin-embed-fallback reveal">
        <p>This LinkedIn post cannot be embedded directly.</p>
        <a class="text-link" href="${post.url}" target="_blank" rel="noopener noreferrer">View post on LinkedIn ${icons.external}</a>
      </article>`;
    }

    const embedUrl = `https://www.linkedin.com/embed/feed/update/urn:li:activity:${activityId}`;

    return `<article class="linkedin-embed-card reveal">
      <iframe
        src="${embedUrl}"
        title="LinkedIn post by ${post.source}"
        loading="lazy"
        frameborder="0"
        allowfullscreen>
      </iframe>
      <a class="linkedin-original-link" href="${post.url}" target="_blank" rel="noopener noreferrer">Open original post on LinkedIn ${icons.external}</a>
    </article>`;
  }

  function mediaPage() {
    const media = data.media;
    const videoUrl = `https://www.youtube-nocookie.com/embed/${media.video.id}`;
    const watchUrl = `https://www.youtube.com/watch?v=${media.video.id}`;

    return `
      ${pageHeader("Media", "Research updates and features", "Selected professional updates, institutional highlights and research-related media.")}

      <section class="section container media-section">
        <div class="media-profile-bar reveal">
          <div>
            <p class="eyebrow">LinkedIn</p>
            <h2>Professional updates</h2>
            <p>Selected posts on recent publications, research developments and institutional features.</p>
          </div>
          <a class="button primary" href="${media.linkedinProfile}" target="_blank" rel="noopener noreferrer">View LinkedIn profile ${icons.external}</a>
        </div>

        <div class="media-group">
          <div class="section-heading compact-heading">
            <p class="eyebrow">Selected posts</p>
            <h2>Research updates</h2>
          </div>
          <div class="media-post-grid">
            ${media.ownPosts.map(mediaPostEmbed).join("")}
          </div>
        </div>

        <div class="media-group">
          <div class="section-heading compact-heading">
            <p class="eyebrow">External features</p>
            <h2>Posts highlighting my work</h2>
          </div>
          <div class="media-post-grid">
            ${media.featuredPosts.map(mediaPostEmbed).join("")}
          </div>
        </div>
      </section>

      <section class="section section-muted">
        <div class="container video-feature reveal">
          ...
          <div class="video-copy">
            ...
            <h2>${media.video.title}</h2>
            <p>${media.video.text}</p>
            <a class="text-link" href="${watchUrl}" target="_blank" rel="noopener noreferrer">Watch on YouTube ${icons.external}</a>
          </div>
        </div>
      </section>
    `;
